...
function updateBossCamera(dt) {
  ...
  function updateFocusButtons() {
    const btnMain = $("focusBtn");
    if (btnMain) btnMain.textContent = `Focus: ${focusOn ? "On" : "Off"}`;

    const btnMini = $("focusMiniBtn");
    if (btnMini) btnMini.textContent = `Click to Focus: ${focusOn ? "On" : "Off"}`;
  }
...
// =========================
  ...
  bind("focus", () => toggleFocus());

  // Option A: extra focus button under stats
  const __miniFocusBtn = $("focusMiniBtn");
  if (__miniFocusBtn) {
    __miniFocusBtn.addEventListener("click", () => { ensureAudio(); toggleFocus(); });
  }
